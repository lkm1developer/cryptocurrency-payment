...
function getBirbPrice($type)
{
	
	if($type=='birb'){
		$wp_options = get_option('woocommerce_custom_settings');
	} else if($type=='busd'){
		$wp_options = get_option('woocommerce_busd_settings');
		
	}
	
	
	$price = get_option($type.'_price');
	$timeatamp = get_option($type.'_timestamp');
	if ($wp_options['show_in_'.$type] == 'yes') {
		...
		if ($timeatamp > time() && floatval($price) > 0) {
			...
			$method = "GET";
			$url = birbBaseUrl . 'payment/rate/'.$type;
			// print($url);
			...
			if ($result['status'] == true) {
				...
				if (floatval($p)) {
					update_option($type.'_timestamp', time() + 60 * 60 * 5);
					update_option($type.'_price', $p);
				}
			}
		}
		$price = get_option($type.'_price');
...
function custom_price_html($defaultprice, $product)
{
	...
	$price = getBirbPrice('birb');
	$price2 = getBirbPrice('busd');
	$pp = $product->get_price();
	$pHtml= $defaultprice;
	if ($price) {
		$coin = number_format(($pp / floatval($price)), 6);
		$pHtml=  $pHtml. ' (' . $coin . ' Birb)';
	}
	if ($price2) {
		$coin = number_format(($pp / floatval($price2)), 6);
		$pHtml= $pHtml . ' (' . $coin . ' BUSD)';
	}
	return $pHtml;
}
function getBirbPriceHtml($defaultprice)
{
	...
	if ($price2) {
		...
		$coin = number_format(($defaultprice / floatval($price2)), 6);
		$p=   $p.' (' . $coin . ' BUSD)';
	}
	return $p;
}
...
function birbSend($data, $token)
{
	...
	$_SESSION['payment_type_crypto'] = 'custom';
...
function correct_redirect()
{
	...
	$payment_type = @$_SESSION['payment_type_crypto'];
	// var_dump($order_id);
	// die;
	global $wpdb;
	$wp_options = $wpdb->get_results("SELECT option_value FROM wp_options WHERE (option_name = 'woocommerce_.$payment_type._settings' AND autoload = 'yes')");
	$option_value = unserialize($wp_options[0]->option_value);
	...
	if ($oder_meta['status'] == 1) {
		...
		wc_add_order_item_meta($order_id, '_'.$payment_type.'_oder_meta', $response);
		$order = wc_get_order($order_id);
		if ($oder_meta['verify']) {
